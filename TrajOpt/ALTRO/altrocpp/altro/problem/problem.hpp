...
class Problem {
 ...
 public:
  ...
  explicit Problem(const int N,
                   std::shared_ptr<VectorXd> initial_state = std::make_shared<VectorXd>(0))
      : N_(N),
        initial_state_(initial_state),
        costfuns_(N + 1, nullptr),
        models_(N + 1, nullptr),
        eq_(N + 1),
        ineq_(N + 1) {}
  ...
  std::shared_ptr<DiscreteDynamics> GetDynamics(int k) const {
    ALTRO_ASSERT((k >= 0) && (k <= N_), "Invalid knot point index.");
    ALTRO_ASSERT(models_[k] != nullptr,
                 fmt::format("Dynamics have not been defined at knot point {}.", k));
    return models_[k];
  }
