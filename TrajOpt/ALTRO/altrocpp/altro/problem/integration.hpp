...
class ExplicitIntegrator : public StateControlSized<NStates, NControls> {
 ...
 public:
   ...
   */
  virtual void Integrate(const DynamicsPtr& dynamics, const VectorXdRef& x, const VectorXdRef& u,
                         float t, float h, Eigen::Ref<VectorXd> xnext) = 0;
   ...
   */
  virtual void Jacobian(const DynamicsPtr& dynamics, const VectorXdRef& x, const VectorXdRef& u,
                        float t, float h, Eigen::Ref<MatrixXd> jac) = 0;
};
...
class ExplicitEuler final : public ExplicitIntegrator<Eigen::Dynamic, Eigen::Dynamic> {
 public:
  ...
  void Integrate(const DynamicsPtr& dynamics, const VectorXdRef& x, const VectorXdRef& u, float t,
                 float h, Eigen::Ref<VectorXd> xnext) override {
    dynamics->Evaluate(x, u, t, xnext);
    xnext = x + xnext * h;
  }
  void Jacobian(const DynamicsPtr& dynamics, const VectorXdRef& x, const VectorXdRef& u, float t,
                float h, Eigen::Ref<MatrixXd> jac) override {
    int n = x.size();
    int m = u.size();
...
class RungeKutta4 final : public ExplicitIntegrator<NStates, NControls> {
 ...
 public:
  RungeKutta4(int n, int m) : ExplicitIntegrator<NStates, NControls>(n, m) { Init(); }
  RungeKutta4() : ExplicitIntegrator<NStates, NControls>() { Init(); }
  void Integrate(const DynamicsPtr& dynamics, const VectorXdRef& x, const VectorXdRef& u, float t,
                 float h, Eigen::Ref<VectorXd> xnext) override {
    dynamics->Evaluate(x, u, t, k1_);
    dynamics->Evaluate(x + k1_ * 0.5 * h, u, t + 0.5 * h,
                       k2_);  // NOLINT(readability-magic-numbers)
    dynamics->Evaluate(x + k2_ * 0.5 * h, u, t + 0.5 * h,
                       k3_);  // NOLINT(readability-magic-numbers)
  ...
  void Jacobian(const DynamicsPtr& dynamics, const VectorXdRef& x, const VectorXdRef& u, float t,
    ...
    dA_[0] = A_[0] * h;
    dA_[1] =
        A_[1] * (MatrixXd::Identity(n, n) + 0.5 * dA_[0]) * h;  // NOLINT(readability-magic-numbers)
    dA_[2] =
        A_[2] * (MatrixXd::Identity(n, n) + 0.5 * dA_[1]) * h;  // NOLINT(readability-magic-numbers)
