...
class Trajectory {
 ...
 public:
  ...
  explicit Trajectory(int _n, int _m, int N) : traj_(N + 1, KnotPoint<n, m, T>(_n, _m)) {}
   ...
   */
  Trajectory(std::vector<VectorN<n, T>> X, std::vector<VectorN<m, T>> U, std::vector<float> times) {
   ...
   */
  bool CheckTimeConsistency(const double eps = 1e-6, const bool verbose = false) {
    for (int k = 0; k < NumSegments(); ++k) {
      ...
      if (std::abs(h_stored - h_calc) > eps) {
        if (verbose) {
          ...
          std::cout << "t-=" << GetTime(k) << "\t t+=" << GetTime(k + 1) << "\t dt=" << h_calc
                    << std::endl;
        }
        return false;
      }
