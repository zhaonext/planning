...
class CostExpansion : public StateControlSized<n, m> {
 public:
  ...
  CostExpansion& operator=(const CostExpansion& exp) {
    if (n > 0) {
      ALTRO_ASSERT(n == exp.StateDimension(), "Invalid copy. State dimension must be consistent.");
    }
    if (m > 0) {
  ...
  void CalcExpansion(const std::shared_ptr<problem::CostFunction>& costfun, const VectorXdRef& x,
                     const VectorXdRef& u) {
