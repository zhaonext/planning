...
class DynamicsExpansion : public StateControlSized<n, m> {
 ...
 public:
  ...
  Eigen::Block<JacType, n, n> GetA() {
    return jac_.template topLeftCorner<n, n>(this->StateDimension(), this->StateDimension());
  }
  Eigen::Block<JacType, n, m> GetB() {
    return jac_.template topRightCorner<n, m>(this->StateDimension(), this->ControlDimension());
  }
  JacType& GetJacobian() { return jac_; };

  template <int n2, int m2, class Dynamics>
  void CalcExpansion(const std::shared_ptr<Dynamics>& model, const KnotPoint<n2, m2>& z) {
    CalcExpansion(model, z.State(), z.Control(), z.GetTime(), z.GetStep());
  }

  void CalcExpansion(const std::shared_ptr<problem::DiscreteDynamics>& model, const VectorXdRef& x,
                     const VectorXdRef& u, const float t, const float h) {
  ...
  void CalcExpansion(const std::shared_ptr<problem::ContinuousDynamics>& model,
                     const VectorXdRef& x, const VectorXdRef& u, const float t, const float h) {
    ALTRO_UNUSED(model);
    ALTRO_UNUSED(x);
    ALTRO_UNUSED(u);
    ALTRO_UNUSED(t);
