...
class KnotPointFunctions : public StateControlSized<n, m> {
 ...
 public:
   ...
   * @return double
   */
  double Cost(const VectorXdRef& x, const VectorXdRef& u) const {
   ...
   */
  void Dynamics(const VectorXdRef& x, const VectorXdRef& u, float t, float h,
   ...
   * @param h step in independent variable
   */
  void CalcDynamicsExpansion(const VectorXdRef& x, const VectorXdRef& u, const float t,
                             const float h) {
    if (model_ptr_) {
   ...
   */
  void CalcActionValueExpansion(const Eigen::Ref<const MatrixXd>& ctg_hessian,
   ...
   */
  void RegularizeActionValue(const double rho, BackwardPassRegularization reg_type =
  ...
  void CalcCostToGo() {
    ...
    ctg_hessian_ = Q.dxdx() + K.transpose() * Q.dudu() * K + K.transpose() * Q.dxdu().transpose()
                   + Q.dxdu() * K;
