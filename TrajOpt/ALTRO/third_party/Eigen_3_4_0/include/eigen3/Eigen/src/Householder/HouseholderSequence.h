...
public:
    ...
    EIGEN_DEVICE_FUNC EIGEN_CONSTEXPR
        Index
        rows() const EIGEN_NOEXCEPT { return Side == OnTheLeft ? m_vectors.rows() : m_vectors.cols(); }
    ...
    EIGEN_DEVICE_FUNC EIGEN_CONSTEXPR
        Index
        cols() const EIGEN_NOEXCEPT { return rows(); }
    ...
    inline EIGEN_DEVICE_FUNC void evalTo(DestType& dst) const {
        Matrix<Scalar, DestType::RowsAtCompileTime, 1,
               AutoAlign | ColMajor, DestType::MaxRowsAtCompileTime, 1>
            workspace(rows());
