...
struct MakeComplex {
    ...
    EIGEN_DEVICE_FUNC
        T
        operator()(const T& val) const { return val; }
};
...
struct MakeComplex<true> {
    ...
    EIGEN_DEVICE_FUNC
        std::complex<T>
        operator()(const T& val) const { return std::complex<T>(val, 0); }
};
...
struct MakeComplex<false> {
    ...
    EIGEN_DEVICE_FUNC
        std::complex<T>
        operator()(const std::complex<T>& val) const { return val; }
};
...
struct PartOf<RealPart> {
    template <typename T>
    T operator()(const std::complex<T>& val) const { return val.real(); }
};

template <>
struct PartOf<ImagPart> {
    template <typename T>
    T operator()(const std::complex<T>& val) const { return val.imag(); }
};
