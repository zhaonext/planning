...
public:
    ...
    Block(XprType& xpr, Index i) : Impl(xpr, i) {
        eigen_assert((i >= 0) && (((BlockRows == 1) && (BlockCols == XprType::ColsAtCompileTime) && i < xpr.rows()) || ((BlockRows == XprType::RowsAtCompileTime) && (BlockCols == 1) && i < xpr.cols())));
    }
    ...
    Block(XprType& xpr, Index startRow, Index startCol)
        ...
        eigen_assert(startRow >= 0 && BlockRows >= 0 && startRow + BlockRows <= xpr.rows() && startCol >= 0 && BlockCols >= 0 && startCol + BlockCols <= xpr.cols());
    }
    ...
    Block(XprType& xpr,
        ...
        eigen_assert(startRow >= 0 && blockRows >= 0 && startRow <= xpr.rows() - blockRows && startCol >= 0 && blockCols >= 0 && startCol <= xpr.cols() - blockCols);
    }
};
...
public:
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const typename internal::remove_all<XprTypeNested>::type& nestedExpression() const {
        return m_xpr;
    }
...
public:
    ...
    BlockImpl_dense(XprType& xpr, Index i)
        : Base(xpr.data() + i * (((BlockRows == 1) && (BlockCols == XprType::ColsAtCompileTime) && (!XprTypeIsRowMajor)) || ((BlockRows == XprType::RowsAtCompileTime) && (BlockCols == 1) && (XprTypeIsRowMajor)) ? xpr.innerStride() : xpr.outerStride()),
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE EIGEN_CONSTEXPR
        StorageIndex
        startRow() const EIGEN_NOEXCEPT { return m_startRow.value(); }

    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE EIGEN_CONSTEXPR
        StorageIndex
        startCol() const EIGEN_NOEXCEPT { return m_startCol.value(); }
