...
struct triangular_solver_selector<Lhs, Rhs, Side, Mode, NoUnrolling, 1> {
    ...
    static EIGEN_DEVICE_FUNC void run(const Lhs& lhs, Rhs& rhs) {
        ...
        triangular_solve_vector<LhsScalar, RhsScalar, Index, Side, Mode, LhsProductTraits::NeedToConjugate,
                                (int(Lhs::Flags) & RowMajorBit) ? RowMajor : ColMajor>::run(actualLhs.cols(), actualLhs.data(), actualLhs.outerStride(), actualRhs);
...
struct triangular_solver_selector<Lhs, Rhs, Side, Mode, NoUnrolling, Dynamic> {
    ...
    static EIGEN_DEVICE_FUNC void run(const Lhs& lhs, Rhs& rhs) {
        ...
        triangular_solve_matrix<Scalar, Index, Side, Mode, LhsProductTraits::NeedToConjugate, (int(Lhs::Flags) & RowMajorBit) ? RowMajor : ColMajor,
                                (Rhs::Flags & RowMajorBit) ? RowMajor : ColMajor, Rhs::InnerStrideAtCompileTime>::run(size, othersize, &actualLhs.coeffRef(0, 0), actualLhs.outerStride(), &rhs.coeffRef(0, 0), rhs.innerStride(), rhs.outerStride(), blocking);
    }
};
...
struct triangular_solver_unroller<Lhs, Rhs, Mode, LoopIndex, Size, false> {
    ...
    static EIGEN_DEVICE_FUNC void run(const Lhs& lhs, Rhs& rhs) {
        if (LoopIndex > 0)
            rhs.coeffRef(DiagIndex) -= lhs.row(DiagIndex).template segment<LoopIndex>(StartIndex).transpose().cwiseProduct(rhs.template segment<LoopIndex>(StartIndex)).sum();
