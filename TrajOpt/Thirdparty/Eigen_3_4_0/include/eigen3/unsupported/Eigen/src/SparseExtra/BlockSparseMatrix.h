...
public:
    // Default constructor
    BlockSparseMatrix()
        : m_innerBSize(0), m_outerBSize(0), m_innerOffset(0), m_outerOffset(0), m_nonzerosblocks(0), m_values(0), m_blockPtr(0), m_indices(0), m_outerIndex(0), m_blockSize(BlockSize) {}
    ...
    BlockSparseMatrix(const BlockSparseMatrix& other)
        : m_innerBSize(other.m_innerBSize), m_outerBSize(other.m_outerBSize), m_nonzerosblocks(other.m_nonzerosblocks), m_nonzeros(other.m_nonzeros), m_blockPtr(0), m_blockSize(other.m_blockSize) {
    ...
    void setFromTriplets(const InputIterator& begin, const InputIterator& end) {
       ...
       */
        ...
        for (InputIterator it(begin); it != end; ++it) {
            ...
            eigen_assert((it->value().rows() == it->value().cols() && (it->value().rows() == m_blockSize)) || (m_blockSize == Dynamic));
...
public:
    InnerIterator(const BlockSparseMatrix& mat, Index outer)
        : m_mat(mat), m_outerB(mat.outerToBlock(outer)), m_outer(outer), itb(mat, mat.outerToBlock(outer)), m_offset(outer - mat.blockOuterIndex(m_outerB)) {
        if (itb) {
