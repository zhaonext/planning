...
public:
    ...
    EIGEN_DEVICE_FUNC
        ...
        operator()(Index firstIndex, IndexTypes... otherIndices) const {
        const std::size_t num_indices = (sizeof...(otherIndices) + 1);
        const array<Index, num_indices> indices{{ firstIndex,
                                                  otherIndices... }};
    ...
    EIGEN_DEVICE_FUNC
        ...
        coeffRef(Index firstIndex, IndexTypes... otherIndices) {
        const std::size_t num_indices = (sizeof...(otherIndices) + 1);
        const array<Index, num_indices> indices{{ firstIndex,
                                                  otherIndices... }};
    ...
    EIGEN_DEVICE_FUNC
        EIGEN_STRONG_INLINE const Scalar
        coeff(const array<Index, NumIndices>& indices) const {
    ...
    EIGEN_DEVICE_FUNC
        EIGEN_STRONG_INLINE Scalar&
        coeffRef(const array<Index, NumIndices>& indices) {
        const Dimensions& dims = this->dimensions();
        Index index = 0;
        if (PlainObjectType::Options & RowMajor) {
            index += indices[0];
