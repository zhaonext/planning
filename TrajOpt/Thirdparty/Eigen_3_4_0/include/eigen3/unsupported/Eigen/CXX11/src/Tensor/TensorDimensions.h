...
struct fixed_size_tensor_index_linearization_helper {
    ...
    EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE Index run(array<Index, NumIndices> const& indices,
        ...
        return array_get < RowMajor ? n - 1 : (NumIndices - n) > (indices) + dget < RowMajor ? n - 1 : (NumIndices - n), Dimensions > ::value *
...
struct fixed_size_tensor_index_extraction_helper {
    template <typename Dimensions>
    EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE Index run(const Index index,
                                                           const Dimensions& dimensions) {
...
struct fixed_size_tensor_index_extraction_helper<Index, 0> {
    template <typename Dimensions>
    EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE Index run(const Index,
                                                           const Dimensions&) {
        return 0;
    }
};
...
template <std::ptrdiff_t V1 = 0, std::ptrdiff_t V2 = 0, std::ptrdiff_t V3 = 0, std::ptrdiff_t V4 = 0, std::ptrdiff_t V5 = 0>
struct Sizes {
...
struct tensor_index_linearization_helper {
    static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
        Index
        run(array<Index, NumIndices> const& indices, array<Index, NumIndices> const& dimensions) {
        return array_get < RowMajor ? n : (NumIndices - n - 1) > (indices) + array_get < RowMajor ? n : (NumIndices - n - 1) > (dimensions)*tensor_index_linearization_helper<Index, NumIndices, n - 1, RowMajor>::run(indices, dimensions);
    }
};
...
struct tensor_index_linearization_helper<Index, NumIndices, 0, RowMajor> {
    static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
        Index
        run(array<Index, NumIndices> const& indices, array<Index, NumIndices> const&) {
...
struct DSizes : array<DenseIndex, NumDims> {
    ...
    EIGEN_DEVICE_FUNC
        EIGEN_STRONG_INLINE explicit DSizes(DenseIndex firstDimension, DenseIndex secondDimension, IndexTypes... otherDimensions) : Base({{ firstDimension,
                                                                                                                                            secondDimension,
                                                                                                                                            otherDimensions... }}) {
...
struct tensor_vsize_index_linearization_helper {
    static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
        Index
        run(array<Index, NumIndices> const& indices, std::vector<DenseIndex> const& dimensions) {
...
struct tensor_vsize_index_linearization_helper<Index, NumIndices, 0, RowMajor> {
    static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
        Index
        run(array<Index, NumIndices> const& indices, std::vector<DenseIndex> const&) {
        return array_get < RowMajor ? 0 : NumIndices - 1 > (indices);
    }
};
}  // end namespace internal
