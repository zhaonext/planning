...
bool Polygon2d::IsPointInRectangle(const Vec2d &point) const {
    if (!CHECK_GE(points_.size(), 3UL)) {
        return false;
    }
...
bool Polygon2d::BuildFromPoints() {
    num_points_ = static_cast<int>(points_.size());
    if (!CHECK_GE(num_points_, 3)) {
        return false;
    }
    ...
    area_ /= 2.0;
    if (!CHECK_GT(area_, kMathEpsilon)) {
        return false;
    }
...
bool Polygon2d::HasOverlapRectangle(const LineSegment2d &line_segment) const {
    if (!CHECK_GE(points_.size(), 3UL)) {
        return false;
    }
    if ((line_segment.start().x() < min_x_ && line_segment.end().x() < min_x_) ||
    ...
    return GetOverlapRectangle(line_segment);
}

bool Polygon2d::GetOverlapRectangle(const LineSegment2d &line_segment) const {
    if (!CHECK_GE(points_.size(), 3UL)) {
        return false;
    }

    if (line_segment.length() <= kMathEpsilon) {
        if (!IsPointInRectangle(line_segment.start())) {
            return false;
        }
        return true;
    }
    ...
    if (line_segment.start().y() == line_segment.end().y()) {
        // 线段平行与x轴
        if (line_segment.start().y() >= min_y_ &&
            line_segment.start().y() <= max_y_) {
            return GetIntervalOverlap(min_x_, max_x_, line_segment.start().x(), line_segment.end().x());
        } else {
            return false;
        }
    }
    ...
    if (IsPointInRectangle(line_segment.end())) {
        return true;
    }
    ...
    // 让b点y坐标大于a点y坐标
    if (line_segment.start().y() > line_segment.end().y()) {
        a_x = line_segment.end().x();
        a_y = line_segment.end().y();
        b_x = line_segment.start().x();
        b_y = line_segment.start().y();
    } else {
        a_x = line_segment.start().x();
        a_y = line_segment.start().y();
        b_x = line_segment.end().x();
        b_y = line_segment.end().y();
    }

    double k = (b_x - a_x) / (b_y - a_y);
    if (a_y < min_y_) {
        c_y = min_y_;
        c_x = k * (c_y - a_y) + a_x;
    } else {
    ...
    if (d_y >= c_y) {
        return GetIntervalOverlap(min_x_, max_x_, d_x, c_x);
    } else {
        return false;
    }
...
bool Polygon2d::GetIntervalOverlap(const double &a, const double &b,
                                   const double &c, const double &d) const {
    // 判断ab和cd是否有交集
    if (std::min(c, d) >= std::max(a, b) ||
...
bool Polygon2d::ExtremePoints(const double heading, Vec2d *const first,
...
Box2d Polygon2d::MinAreaBoundingBox() const {
    ...
    if (!is_convex_) {
        ...
        if (!CHECK(convex_polygon.is_convex())) {
            return Box2d();
        }
...
Polygon2d Polygon2d::ExpandByDistance(const double distance) const {
    ...
    if (!CHECK(ComputeConvexHull(points, &new_polygon))) {
        return Polygon2d();
    }
    return new_polygon;
}
...
std::string Polygon2d::DebugString() const {
    std::string tmp = "polygon2d (  num_points = " + std::to_string(num_points_) + " )  ";
    return tmp;
}