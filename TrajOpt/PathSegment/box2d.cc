...
Box2d::Box2d(const Vec2d &center, const double heading, const double length,
             const double width)
    : center_(center), length_(length), width_(width), half_length_(length / 2.0), half_width_(width / 2.0), heading_(heading), cos_heading_(cos(heading)), sin_heading_(sin(heading)) {
    CHECK_GT(length_, -kMathEpsilon);
    CHECK_GT(width_, -kMathEpsilon);
    InitCorners();
}

void Box2d::Reset(const Vec2d &center, const double heading, const double length,
                  const double width) {
    center_ = center;
    heading_ = heading;
    length_ = length;
...
inline void Compare4Values(const double &x1, const double &x2, const double &x3, const double &x4, double &max_v, double &min_v) {
    double max12, max34, min12, min34;
    if (x1 > x2) {
        max12 = x1;
        min12 = x2;
    } else {
    ...
    max_v = std::fmax(max12, max34);
    min_v = std::fmin(min12, min34);
}

void Box2d::ResetPartial(const Vec2d &center, const double cos_heading, const double sin_heading, const double length,
                         const double width) {
    // will not update corners, use careful.
    // can be use for update for overlap check, overlap check need not for corners
    center_ = center;
    ...
    auto x1 = center_.x() + dx1 + dx2;
    auto x2 = center_.x() + dx1 - dx2;
    auto x3 = center_.x() - dx1 - dx2;
    auto x4 = center_.x() - dx1 + dx2;
    auto y1 = center_.y() + dy1 + dy2;
    auto y2 = center_.y() + dy1 - dy2;
    auto y3 = center_.y() - dy1 - dy2;
    auto y4 = center_.y() - dy1 + dy2;

    Compare4Values(x1, x2, x3, x4, max_x_, min_x_);
    Compare4Values(y1, y2, y3, y4, max_y_, min_y_);
}

void Box2d::ResetPartial(const Vec2d &center, const double cos_heading, const double sin_heading) {
    // will not update corners, use careful.
    // can be use for update for overlap check, overlap check need not for corners
    center_ = center;
    cos_heading_ = cos_heading;
...
Box2d::Box2d(const LineSegment2d &axis, const double width)
    : center_(axis.center()), length_(axis.length()), width_(width), half_length_(axis.length() / 2.0), half_width_(width / 2.0), heading_(axis.heading()), cos_heading_(axis.cos_heading()), sin_heading_(axis.sin_heading()) {
    CHECK_GT(length_, -kMathEpsilon);
    CHECK_GT(width_, -kMathEpsilon);
    InitCorners();
}
...
void Box2d::InitCorners() {
    ...
    const double dx2 = sin_heading_ * half_width_;
    const double dy2 = -cos_heading_ * half_width_;
    corners_[0].set_x(center_.x() + dx1 + dx2);
    corners_[0].set_y(center_.y() + dy1 + dy2);
    corners_[1].set_x(center_.x() + dx1 - dx2);
    corners_[1].set_y(center_.y() + dy1 - dy2);
    corners_[2].set_x(center_.x() - dx1 - dx2);
    corners_[2].set_y(center_.y() - dy1 - dy2);
    corners_[3].set_x(center_.x() - dx1 + dx2);
    corners_[3].set_y(center_.y() - dy1 + dy2);
...
Box2d::Box2d(const AABox2d &aabox)
    : center_(aabox.center()), length_(aabox.length()), width_(aabox.width()), half_length_(aabox.half_length()), half_width_(aabox.half_width()), heading_(0.0), cos_heading_(1.0), sin_heading_(0.0) {
    CHECK_GT(length_, -kMathEpsilon);
    CHECK_GT(width_, -kMathEpsilon);
}
...
struct xVector2d {
    ...
    friend void swap(xVector2d &lhs, xVector2d &rhs) {
        std::swap(lhs.m_x, rhs.m_x);
        std::swap(lhs.m_y, rhs.m_y);
    }

    xVector2d operator+(const xVector2d &other) const {
    ...
    xVector2d operator*(double c) const {
        return {m_x * c, m_y * c};
    }
...
bool Box2d::HasOverlap(const LineSegment2d &line_segment) const {
    ...
    auto sin_phi_n = -sin_heading_;
    auto cos_phi_n = cos_heading_;

    auto ax_raw = line_segment.start().x() - center_.x();
    auto ay_raw = line_segment.start().y() - center_.y();
    auto bx_raw = line_segment.end().x() - center_.x();
    auto by_raw = line_segment.end().y() - center_.y();

    auto ax = cos_phi_n * ax_raw - sin_phi_n * ay_raw;
    auto ay = sin_phi_n * ax_raw + cos_phi_n * ay_raw;
    auto bx = cos_phi_n * bx_raw - sin_phi_n * by_raw;
    auto by = sin_phi_n * bx_raw + cos_phi_n * by_raw;
    ...
    auto quick_check = [&](double x, double y) -> bool {
        if (x <= half_length_ and x >= -half_length_ and y <= half_width_ and y >= -half_width_)
            return true;
        return false;
    };

    // check any end point in box
    if (quick_check(ax, ay) || quick_check(bx, by)) {
        return true;
    }
     ...
     *  D---------C         Y
     */
    xVector2d XY(bx - ax, by - ay);
    xVector2d XA(-half_length_ - ax, half_width_ - ay);
    xVector2d XB(half_length_ - ax, half_width_ - ay);
    xVector2d XC(half_length_ - ax, -half_width_ - ay);
    xVector2d XD(-half_length_ - ax, -half_width_ - ay);

    bool flag_a = XY.cross(XA) * XY.cross(XC) < 0;
    bool flag_b = XY.cross(XB) * XY.cross(XD) < 0;
...
double Box2d::DistanceTo(const LineSegment2d &line_segment) const {
    ...
    AFATAL << "unimplemented state: " << gx1 << " " << gy1 << " " << gx2 << " "
           << gy2;
    return 0.0;
}
...
bool Box2d::HasOverlapAABB(const Box2d &box) const {
    // if (box.max_x() < min_x() || box.min_x() > max_x() || box.max_y() < min_y() ||
    //     box.min_y() > max_y()) {
    //     return false;
    // }

    // 第一个物体的最右侧是否大于第二个物体的最左侧并且第二个物体的最右侧是否大于第一个物体的最左侧；
    bool collisionX = box.max_x() >= min_x() && max_x() >= box.min_x();
    bool collisionY = box.max_y() >= min_y() && max_y() >= box.min_y();
...
AABox2d Box2d::GetAABox() const {
    ...
    return {center_, (dx1 + dx2) * 2.0, (dy1 + dy2) * 2.0};
}
...
std::string Box2d::DebugString() const {
    std::string tmp = "box2d ( center = " + center_.DebugString() + "  heading = " + std::to_string(heading_) + "  length = " + std::to_string(length_) + "  width = " + std::to_string(width_) + "  cos_heading = " + std::to_string(cos_heading_) + "  sin_heading = " + std::to_string(sin_heading_) + "  min_x = " + std::to_string(min_x_) + "  max_x = " + std::to_string(max_x_) + "  min_y = " + std::to_string(min_y_) + "  max_y = " + std::to_string(max_y_) + " )";
    return tmp;
}
