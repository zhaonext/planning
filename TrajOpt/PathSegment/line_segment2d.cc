...
void LineSegment2d::Init(const Vec2d &start, const Vec2d &end) {
    start_ = start;
    end_ = end;
    const double dx = end_.x() - start_.x();
    const double dy = end_.y() - start_.y();
...
std::string LineSegment2d::DebugString() const {
    std::string tmp = "segment2d ( start = " + start_.DebugString() + "  end = " + end_.DebugString() + " )";
    return tmp;
}