...
void Vec2d::SelfRotate(const double cos_angle, const double sin_angle) {
    double tmp_x = x_;
    x_ = x_ * cos_angle - y_ * sin_angle;
    y_ = tmp_x * sin_angle + y_ * cos_angle;
}
...
Vec2d Vec2d::operator/(const double ratio) const {
    if (!CHECK_GT(std::abs(ratio), kMathEpsilon)) {
        return Vec2d();
    }
    return Vec2d(x_ / ratio, y_ / ratio);
}
...
std::string Vec2d::DebugString() const {
    std::string tmp = "vec2d ( x = " + std::to_string(x_) + "  y = " + std::to_string(y_) + " )";
    return tmp;
}
